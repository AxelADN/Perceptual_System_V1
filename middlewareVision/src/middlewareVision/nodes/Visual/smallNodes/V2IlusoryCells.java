...
public class V2IlusoryCells extends FrameActivity {
    ...
    public V2IlusoryCells() {
        ...
        edges = new Mat[Config.gaborOrientations];
        initFrames(4, 12);
    }
    ...
    public void receive(int nodeID, byte[] data) {
        try {
            ...
            if (spike.getModality() == Modalities.VISUAL) {
                ...
                edges[index] = Convertor.matrixToMat((matrix) spike.getIntensity());
            ...
            if (sync.isComplete()) {
                ...
                ilusoryEdges = elongatedGaborFilter(edges, sigma*0.08f, 0, 31,20,0.1);
                }
                else{
                ilusoryEdges=edges;
                }
                ...
                for (int i = 0; i < Config.gaborOrientations; i++) {
                    if(Config.ilusoryEnabled)
                    ilusoryEdges[i]=MatrixUtils.maxSum(ilusoryEdges[i],edges[i]);
                    frame[i].setImage(Convertor.ConvertMat2Image(ilusoryEdges[i]), "ilusory " + i);
                    LongSpike sendSpike = new LongSpike(Modalities.VISUAL, new Location(i), Convertor.MatToMatrix(ilusoryEdges[i]), 0);
     ...
     * @param aspectRatio   <0.5 elongated
     * @return 
     */
    public Mat[] elongatedGaborFilter(Mat img[], float sigma, double psi, int kernelSize, double lenght, double aspectRatio) {
        Mat ors[];
        ...
        for (int i = 0; i < Config.gaborOrientations; i++, angle += inc) {
            ...
            ors[i] = Mat.zeros(img[i].rows(), img[i].cols(), CvType.CV_32FC1);
            Mat gab = Mat.zeros(img[i].rows(), img[i].cols(), CvType.CV_32FC1);
           ...
           // Imgproc.getga
            ...
            Imgproc.filter2D(img[i], gab, CV_32F, kernel);
