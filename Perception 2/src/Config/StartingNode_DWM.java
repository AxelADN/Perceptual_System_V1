 ...
 * @author AxelADN
 */
public class StartingNode_DWM extends ProcessTemplate implements FrameNodeInterface, AreaBridgeListener {

    private int time;
    private static GUI_DWM gui = null;
    private Mat img;
    private byte[] imgBytes;
    private BigNodeBridge nodeBridge;
    private int experimentNumber = 40;

    public StartingNode_DWM() {
    ...
    public void init() {
        ...
        java.awt.EventQueue.invokeLater(() -> {
            if (!SystemConfig.TRAINING_MODE) {
                if (gui == null) {
                    gui = new GUI_DWM(StartingNode_DWM.this);
    ...
    private void trainningCycle() {
        for (int i = 1; i <= 101; i++) {
            try {
                ...
                Logger.getLogger(StartingNode_DWM.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
